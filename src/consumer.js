...
async function updateInformix (payload) {
    ...
    case 'insert':
      {
	   //const columns = payload.data
        const columns = payload.payload
        const columnNames = Object.keys(columns)
        sql = `insert into ${payload.schema}:${payload.table} (${columnNames.join(', ')}) values (${columnNames.map((k) => `'${columns[k]}'`).join(', ')});` // "insert into <schema>:<table> (col_1, col_2, ...) values (val_1, val_2, ...)"
      }
      break
    case 'update':
      {
        ...
        sql = `update ${payload.schema}:${payload.table} set ${Object.keys(columns).map((key) => `${key}='${columns[key]}'`).join(', ')} where ${primaryKey}=${columns[primaryKey]};` // "update <schema>:<table> set col_1=val_1, col_2=val_2, ... where primary_key_col=primary_key_val"
      }
      break
    case 'delete':
      {
        ...
        const columns = payload.payload
        const primaryKey = payload.Uniquecolumn
        sql = `delete from ${payload.schema}:${payload.table} where ${primaryKey}=${columns[primaryKey]};` // ""delete from <schema>:<table> where primary_key_col=primary_key_val"
      }
      break
    default:
  ...
  const result = await informix.executeQuery(payload.schema, sql, null)
  return result
}
